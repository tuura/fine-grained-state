{-# LANGUAGE LambdaCase, GADTs, TypeFamilies, RankNTypes, FlexibleContexts #-}

module Applications.ISA.SimulatorIO (
...
type IOMachineState = IORef (MachineValue, MachineValue)
...
readKey :: (MonadReader IOMachineState m, MonadIO m) =>
    MachineKey a -> m a
readKey key = case key of
    Reg  reg  -> do env <- ask
                    liftIO (fst <$> readIORef env)
...
writeKey :: (MonadReader IOMachineState m, MonadIO m) =>
         MachineKey a
         -> m a
         -> m a
writeKey k fv = case k of
    Reg  reg  -> do env <- ask
                    v <- fv
                    (a, b) <- liftIO $ readIORef env
                    liftIO $ writeIORef env (a, v)
                    liftIO $ pure v
