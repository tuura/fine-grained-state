...
module Applications.ISA.Symbolic.SymEngine where
...
readProgram :: InstructionAddress -> SymEngine Instruction
readProgram addr = do
    currentState <- getState
    delay 1
    pure . snd $ (!!) (program currentState) (fromIntegral addr)

readInstructionRegister :: SymEngine Instruction
readInstructionRegister = instructionRegister <$> getState

writeInstructionRegister :: Instruction -> SymEngine ()
...
--   Applicative.
setA :: Register -> MachineValue -> SymEngine ()
setA reg simm =
